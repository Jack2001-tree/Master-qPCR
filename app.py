...
if 'results' not in st.session_state:
    ...
    with col_source:
        ...
        plot_title_input = st.text_input("图表标题 (留空默认使用基因名)", "")
        
        tab_geom, tab_theme, tab_stats = st.tabs(["图形设置", "坐标轴与字体", "显著性与图例"])
        ...
        with tab_theme:
            ...
            xlab_text = col_t1.text_input("X轴标题", "Groups")
            ylab_text = col_t2.text_input("Y轴标题", "Relative Expression")
        ...
        with tab_stats:
            ...
            legend_title = st.text_input("图例标题", "Condition")
    ...
    with col_viewer:
        ...
        leg_kwargs = {"title": legend_title, "frameon": False, "fontsize": legend_text_size, "title_fontsize": legend_text_size}
        ...
        fig, ax = plt.subplots(figsize=(num_w, num_h))
        ...
        if plot_type == "bar":
            for i, px in enumerate(safe_x_order):
                ...
                for j, hue in enumerate(hue_levels):
                    ...
                    if len(group_data) > 0:
                        ...
                        if show_points:
                            jitter = np.random.uniform(-jitter_width/2, jitter_width/2, size=len(group_data))
                            face_col = "none" if point_fill_col.lower() == "none" else point_fill_col
            ...
            if plot_type == "box": sns.boxplot(data=dat, x='PLOT_X', y='PLOT_Y', hue='PLOT_FILL', order=safe_x_order, hue_order=hue_levels, dodge=is_grouped, width=current_width, boxprops={'edgecolor':'black', 'linewidth':axis_lwd}, ax=ax, palette=final_cols)
                ...
                if len(safe_x_order) > 2:
                    try:
                        ...
                        for i, px in enumerate(safe_x_order):
                            ...
                            safe_top = bar_stat['mean'].values[0] + (bar_stat['std'].values[0] if pd.notna(bar_stat['std'].values[0]) else 0) if plot_type == 'bar' else bar_stat['max'].values[0]
                            ax.text(i, safe_top + base_offset, letters_dict.get(str(px), ""), ha='center', va='bottom', color='black', fontsize=sig_size)
                    except Exception: pass
                    ...
                    try:
                        ...
                        if len(g1)>1 and len(g2)>1:
                            ...
                            sig = "***" if p_val < 0.001 else "**" if p_val < 0.01 else "*" if p_val < 0.05 else "ns"
                            stack_y = local_max + base_offset
                            ax.plot([0, 0, 1, 1], [stack_y, stack_y+tip, stack_y+tip, stack_y], lw=sig_lwd, c='black')
                            ax.text(0.5, stack_y+tip + (max_y_overall*0.01), sig, ha='center', va='bottom', color='black', fontsize=sig_size)
                    except Exception: pass

        # 图表坐标与边框美化
        ax.set_title(plot_title_input if plot_title_input else selected_gene, pad=20, fontsize=title_size, loc='center', fontstyle=title_style if title_style!='bold' else 'normal', fontweight='bold' if title_style=='bold' else 'normal')
        ax.set_ylabel(ylab_text, fontsize=ytitle_size)
        ax.set_xlabel(xlab_text, fontsize=xtitle_size)
        
        final_x_labels = [x_rename_dict.get(orig_x, orig_x) for orig_x in safe_x_order]
        ax.set_xticks(range(len(safe_x_order)))
        if x_angle > 0: ax.set_xticklabels(final_x_labels, rotation=x_angle, ha="right", rotation_mode="anchor", fontsize=xtext_size)
        else: ax.set_xticklabels(final_x_labels, rotation=0, ha="center", fontsize=xtext_size)
            
        ax.tick_params(axis='y', labelsize=ytext_size)
        sns.despine()
        ax.spines['bottom'].set_linewidth(axis_lwd)
        ax.spines['left'].set_linewidth(axis_lwd)
        ax.tick_params(width=axis_lwd)
